...
import type { ActivityAttempt, AssignedActivity, ClassActivity, Classroom, RankedStudent, Student, StudentAccessProvision, TeacherDashboard } from '../types/platform'
...
export interface PlatformRepository {
  ...
  join(classCode: string, accessCode: string, displayName?: string): Promise<Student>
  ...
  addStudent(classId: string, displayName: string, code?: string): Promise<StudentAccessProvision>
  ...
  setClassActivity(classId: string, activityId: string, patch: Pick<ClassActivity, 'isFeatured' | 'availableFrom' | 'availableUntil'>): Promise<ClassActivity>
  dashboard(classId?: string): Promise<TeacherDashboard>
  ranking(classId: string, activityId: string, currentStudentId?: string): Promise<RankedStudent[]>
...
class LocalRepositoryAdapter implements PlatformRepository {
  ...
  async listAssignedActivities(student: Student) { return this.repository.listAssignedActivities(student) }
  async getActivity(activityId: string) { return this.repository.getActivity(activityId) }
  async join(classCode: string, accessCode: string, displayName?: string) { return this.repository.join(classCode, accessCode, displayName) }
  async getStudent(studentId: string) { return this.repository.getStudent(studentId) }
  async listStudentAttempts(studentId: string) { return this.repository.listStudentAttempts(studentId) }
  ...
  async createClass(name: string, code: string) { return this.repository.createClass(name, code) }
  async addStudent(classId: string, displayName: string, code?: string) { return this.repository.addStudent(classId, displayName, code) }
  async resetStudentAccess(studentId: string) { return this.repository.resetStudentAccess(studentId) }
  async createActivity(activity: Activity, classId: string) { return this.repository.createActivity(activity, classId) }
  async updateActivity(activity: Activity) { return this.repository.updateActivity(activity) }
  async archiveActivity(activityId: string) { this.repository.archiveActivity(activityId) }
  async setClassActivity(classId: string, activityId: string, patch: Pick<ClassActivity, 'isFeatured' | 'availableFrom' | 'availableUntil'>) { return this.repository.setClassActivity(classId, activityId, patch) }
  async dashboard(classId?: string) { return this.repository.dashboard(classId) }
  async ranking(classId: string, activityId: string, currentStudentId?: string) { return this.repository.ranking(classId, activityId, currentStudentId) }
